...
//| ExpertAdvisorHelper v1.0.0                Copyright 2017, Lukkou |
...
public:
    ...
    ///param name="maxPosition":最大ポジション数
    ///param name="spreadFilter":最大スプレット
    ///param name="timeout":タイムアウト
    ///param name="isCountdown":カウントダウン方式
    ...
    // 発注する
    ///param name="cmd":売買種別
    ///param name="volume":売買ロット
    ///param name="price":価格
    ///param name="slippage":許容スリッピング（Pips単位）
    ///param name="stoploss":ストップロス（Pips単位）
    ///param name="takeprofit":利確値（Pips単位）
    ///param name="comment":コメント
    ///param name="expiration":注文有効期限
    ///param name="arrowColor":注文矢印の色
    ...
    // 指定番号の現在利益を取得する。
    // Return   現在利益(Pips)
    double GetOrderProfit(int index);
   
    //------------------------------------------------------------------
    // 指定番号の現在リミット値を取得する。
    // Return   リミット値
    double GetOrderLimit(int index);
    ...
    // 指定番号の現在値を取得する。
    // Return   現在価格
    double GetOrderClose(int index);
   
    //------------------------------------------------------------------
    // 指定番号の取引開始時間 を取得する。
    // Return   取引開始時間 
    datetime GetOrderOpenTime(int index);
    ...
    double GetPipsValue()
    {
        return m_point * m_pipsRate;
    };

    //------------------------------------------------------------------
    // 指定pipsあたりの値を取得する。
    // Return   指定pipあたりの値
    double GetPipsValue(double pips)
    {
    ...
    while(!IsStopped())
    {
        ...
        if(IsTradeAllowed())
        {
            ...
            if(nowStop > 0 ) 
            {
                ...
                if((cmd == OP_BUY && stoplossValue <= OrderStopLoss()) || (cmd == OP_SELL && stoplossValue >= OrderStopLoss()))
                {
                  stoplossValue = nowStop;
                } 
            }
...
)
{
    ...
    while(!IsStopped())
    {
        ...
        if(IsTradeAllowed())
        {
            ...
            {
                // 注文ストップ値は下方方向への変更は許可しない。
                if((cmd == OP_BUY && stoplossValue <= nowStop) || (cmd == OP_SELL && stoplossValue >= nowStop))
                {
                    stoplossValue = nowStop ;
                }
            }
...
// 指定番号のチケットを取得する。
// Return   チケット番号 取得失敗時-1
int ExpertAdvisorTradeHelper::GetTicket(int index)
{
...
// 指定番号の現在利益を取得する。
// Return   現在利益(Pips)
double ExpertAdvisorTradeHelper::GetOrderProfit(int index)
{
    int ticket = GetTicket(index);
...
// 指定番号の現在リミット値を取得する。
// Return   リミット値
double ExpertAdvisorTradeHelper::GetOrderLimit(int index)
{
    int ticket = GetTicket(index);
    if( ticket < 0 ) return 0;
    if( OrderSelect(ticket, SELECT_BY_TICKET) )
...
// 指定番号の現在リミット値を取得する。
// Return   リミット値
double ExpertAdvisorTradeHelper::GetOrderLimitPips(int index)
{
    int ticket = GetTicket(index);
    if( ticket < 0 ) return 0;
    if( OrderSelect(ticket, SELECT_BY_TICKET) )
...
// 指定番号の現在値を取得する。
// Return   現在価格
double ExpertAdvisorTradeHelper::GetOrderClose(int index)
{
    int ticket = GetTicket(index);
    if( ticket < 0 ) return 0;
    if( OrderSelect(ticket, SELECT_BY_TICKET) )
...
// 指定番号のオープン価格を取得する。。
// Return   オープン値
double ExpertAdvisorTradeHelper::GetOrderOpen(int index)
{
    int ticket = GetTicket(index);
    if( ticket < 0 ) return 0;
    if( OrderSelect(ticket, SELECT_BY_TICKET) )
...
// 指定番号の取引開始時間 を取得する。
// Return   取引開始時間 
datetime ExpertAdvisorTradeHelper::GetOrderOpenTime(int index)
{
    int ticket = GetTicket(index);
    if( ticket < 0 ) return 0;
    if( OrderSelect(ticket, SELECT_BY_TICKET) )
